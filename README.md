...
Inclui, entre outras funções: tipos de formulário e perguntas (formulários dinâmicos; **TEXTO** com até 5000 caracteres e linhas configuráveis no lançamento), **pessoas** (com endereço), **cidades** e **bairros**, **lançamento** de respostas (campos opcionais), **entrevista com o assistido** (6 abas), **consulta com exportação PDF** (layout compacto + **Pág. N** no canto superior direito), usuários, permissões e **módulos do sistema** (código numérico) para organizar o menu.
...
- Visão geral, arquitetura e modelo de dados (incl. **auditoria**, **cidades/bairros** com código **inteiro sequencial automático**, pessoa ampliada e **módulos** com código numérico)
...
- Login, usuários, tipos e permissões por programa
- **Auditoria** (usuário e data/hora UTC de inclusão/alteração), tecla **Enter** nos formulários e **seletor com pesquisa** (bairro/município no cadastro de pessoa)
- Regras de projeto (Arial, sem delete em cascata) em `.cursor/rules/`
- Configuração, histórico e troubleshooting
...
2. **Tipos de formulário** → **Cadastrar perguntas** → **Cidades** / **Bairros** (opcional) → **Cadastrar pessoas**
3. **Lançamento** — tipo → pesquisar pessoa → preencher o que quiser (nada é obrigatório) → enviar
...
6. **Administração** — usuários, tipos, liberação de acesso, **módulos do sistema**
...
| Recurso | Rotas |
|---------|--------|
| Módulos do sistema | `GET /modulos-sistema/menu`, CRUD `/modulos-sistema`, `PUT …/programas` |
| Tipos de formulário | `/tipos-formulario` |
| Perguntas | `/perguntas` (`?opcoesAtivas=true` no lançamento) |
| Pessoas | `/pessoas` (`?nome=`, `?cpf=`, `?rg=`) |
